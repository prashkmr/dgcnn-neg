...
import torch.optim as optim # top_loss_out     = top_batch_cost(logits.detach().cpu(), diagramlayerToplevel, F)
...
def train(args, io):
    ...
    for epoch in range(0, 100):
        ...
        # k = 0
        # k = (k+1)%2
        for dataloader in trainloaders[epoch%2]:
        ...
        test_pred = []
        test_true = []
        for dataloader in test_loader[0]:
            for data in dataloader:
                dynamic_data = data[1].to(device)
                static_data = data[2].to(device)
                batch_size = dynamic_data.size()[0]
