...
def getTopoloss(lidar, latent):
    x_distances = compute_distance_matrix(lidar)
    latent_norm = torch.nn.Parameter(torch.ones(1), requires_grad=True).cuda()
    dimensions = lidar.size()
    if len(dimensions) == 4:
    ...
    loss = TopologicalSignatureDistance()
    topo_error, topo_error_components = loss(x_distances.cuda(), latent_distances.cuda())
    ...
    for epoch in trange(0, 100):
        ...
        for dataloader in train_loader:
            for data in dataloader:
                ...
                #topoError1 = getTopoloss(data[2], hidden1)
                #topoError2 = getTopoloss(data[2], hidden2)
                topoError3 = getTopoloss(static_data.reshape(-1,3,40,128), z)
                # print(topoError1, topoError2, topoError3)
                # exit(0)
...
def train_kitti(args, io):
    ...
    trainloaders = load_kitti_DyToSt(npy, skip, headstart)
    npy = [3]
    skip = [1]
    headstart = [0]
    testloaders = load_kitti_DyToSt(npy, skip, headstart)
