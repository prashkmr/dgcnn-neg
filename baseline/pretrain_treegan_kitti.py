...
from data.CRN_dataset import KITTI_loader, load_kitti_DyToSt
...
class TreeGAN():
    def __init__(self, args):
        ...
        ### dataset
        #-----------------------------------------------------------------------------------------------------------------
        # Code only for KITTI currently to allow for alternating between loader. Ideally this should have bee ndone for CARLA, too. lol
        ...
        self.dataloader1 = trainloaders[0]
        self.dataloader2 = trainloaders[1]
        #-----------------------------------------------------------------------------------------------------------------
        #This was the original code
        # self.data = KITTI_loader(args)
        #self.dataLoader1 = torch.utils.data.DataLoader(self.data, batch_size=args.batch_size, shuffle=True, pin_memory=True, num_workers=12)
        # print("Training Dataset : {} prepared.".format(len(self.data)))
        #-----------------------------------------------------------------------------------------------------------------

        print("Training Dataset : {} prepared.".format(len(self.dataloader1)))
    ...
    def run(self, save_ckpt=None, load_ckpt=None):        
        ...
        for epoch in range(epoch_log, self.args.epochs):
            ...
            if epoch % 2 ==0:
                dataloader = self.dataloader1
            else:
                dataloader = self.dataloader2
            ...
            if self.args.eval_every_n_epoch > 0:
                if epoch % self.args.eval_every_n_epoch == 0 :
                    # checkpoint_eval(self.G, self.args.device, n_samples=2048, batch_size=self.args.batch_size,conditional=False, ratio='even', FPD_path=self.args.FPD_path,class_choices=self.args.class_choice)
                    pass
